...
{
    UIView * alertSquare = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 250, 180)];
    ...
    [alertSquare.layer setCornerRadius:2];
...
{
    UIView * circleMask = [[UIView alloc]initWithFrame:CGRectMake([self screenFrame].size.width/2, (([self screenFrame].size.height/2)-alertView.frame.size.height/2) , 80, 80)];
    circleView = [[AMBouncingView alloc]initSuccessCircleWithFrame:CGRectMake(0, 0, 0, 0) andImageSize:80 forAlertType:type andColor:color];
    
    _logoView = [[UIImageView alloc]initWithFrame:CGRectMake(circleMask.frame.size.width/2-40, circleMask.frame.size.height/2-40 , 0, 0)];
...
- (void) labelSetupWithTitle:(NSString*) title andText:(NSString*) text
{
    ...
    _titleLabel.font = [UIFont fontWithName:@"GothamRounded-Book" size:20];
    _titleLabel.textAlignment = NSTextAlignmentCenter;
    
    [alertView addSubview:_titleLabel];
    
    
    _textLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 180, 58)];
    _textLabel.center = CGPointMake(alertView.frame.size.width/2, 80);
    _textLabel.text = text;
    _textLabel.font = [UIFont fontWithName:@"GothamRounded-Book" size:16];
...
- (void) buttonSetupForType:(AlertType)type withCancelButton:(BOOL) hasCancelButton andColor:(UIColor*) color
{
    ...
    if (hasCancelButton) {
        //default button
        _defaultButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 84, 44)];
        _defaultButton.center = CGPointMake((alertView.frame.size.width/4)+3, buttonY);

        //cancel button
        _cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 84, 44)];
        _cancelButton.center = CGPointMake((alertView.frame.size.width*3/4)-3, buttonY);
        ...
        _cancelButton.titleLabel.font = [UIFont fontWithName:@"GothamRounded-Book" size:[UIFont buttonFontSize]];
        ...
        _defaultButton.center = CGPointMake(alertView.frame.size.width/2, buttonY);
    }
    ...
    _defaultButton.titleLabel.font = [UIFont fontWithName:@"GothamRounded-Book" size:[UIFont buttonFontSize]];
...
- (void) circleAnimation
{
    ...
    [animationBlocks addObject:^(BOOL finished){;
        [UIView animateWithDuration:0.15 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            _logoView.frame = [self newFrameForView:_logoView withWidth:60 andHeight:60];
        } completion: getNextAnimation()];
    }];
...
- (void) dismissAlertView
{
    ...
    NSMutableArray* dismissBlocks = [NSMutableArray new];
    
    typedef void(^dismissBlock)(BOOL);
    
    // getNextAnimation
    // removes the first block in the queue and returns it
    dismissBlock (^getNextAnimation)() = ^{
        dismissBlock block = dismissBlocks.count ? (dismissBlock)[dismissBlocks objectAtIndex:0] : nil;
        if (block){
            [dismissBlocks removeObjectAtIndex:0];
            return block;
        }else{
            return ^(BOOL finished){};
        }
    ...
    //block 1
    [dismissBlocks addObject:^(BOOL finished){;
        [UIView animateWithDuration:0.15 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            circleView.frame = [circleView newFrameWithWidth:0 andHeight:0];
            _logoView.frame = [self newFrameForView:_logoView withWidth:0 andHeight:0];
        } completion: getNextAnimation()];
    }];
