...
@interface PNBarChart () {
    ...
    NSMutableArray *_topChartLabels;
...
- (void)setBarTopLabels:(NSArray *)topLabels
{
    _topLabels = topLabels;
    
    if (_topChartLabels) {
        [self viewCleanupForCollection:_topChartLabels];
    }else{
        _topChartLabels = [NSMutableArray new];
    }
    
    if (_showLabel) {
        _xLabelWidth = (self.frame.size.width - _chartMargin * 2) / [topLabels count];
        int labelAddCount = 0;
        for (int index = 0; index < _xLabels.count; index++) {
            labelAddCount += 1;
            
            if (labelAddCount == _xLabelSkip) {
                NSString *labelText = [_topLabels[index] description];
                PNChartLabel * label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0, 0, _xLabelWidth, kXLabelHeight)];
                label.font = _labelFont;
                label.textColor = _labelTextColor;
                [label setTextAlignment:NSTextAlignmentCenter];
                ...
                label.center = CGPointMake(labelXPosition,
                                           self.frame.size.height - kXLabelHeight - _chartMargin + label.frame.size.height /2.0 + _labelMarginTop - _chartTopMargin);
                labelAddCount = 0;
                
                [_topChartLabels addObject:label];
                [self addSubview:label];
            }
        }
    }
...
- (void)updateBar
{
    ...
    //Add bars
    CGFloat chartCavanHeight = self.frame.size.height - _chartMargin * 2 - kXLabelHeight - _chartTopMargin;
    NSInteger index = 0;
    ...
    [UIView animateWithDuration:1 delay:0 options:UIViewAnimationOptionCurveEaseInOut
                     animations:^(void) {
                         for (PNChartLabel *topLabel in _topChartLabels) {
                             NSNumber *valueString = [_yValues objectAtIndex:[_topChartLabels indexOfObject:topLabel]];
                             float value = [valueString floatValue];
                             float grade = (float)value / (float)_yValueMax;
                             
                             topLabel.center = CGPointMake(topLabel.center.x,
                                                        topLabel.center.y - grade*chartCavanHeight);
                         }
                     }
                     completion:nil];
}
